...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (!data.error) {
          data.mode === "tournament"
            ...
            : data.mode === "1vs1"
            ? navigate("/mainpage/game/solo/1vs1/random")
            : navigate("/mainpage/game/solo/2vs2/random");
        }
      } catch (error) {
        console.error(
    ...
    if (
      user &&
      (location.pathname === "/mainpage/game/solo" ||
  ...
  useEffect(() => {
    const set_is_inside = async (flag) => {
      const response = await fetch(
        `http://${import.meta.env.VITE_IPADDRESS}:8000/api/set-is-inside`,
        {
          method: "POST",
          credentials: "include",
          headers: {
    ...
    if (user) {
      if (
        location.pathname !== "/mainpage/game/createtournament" &&
        location.pathname !== "/mainpage/game/tournamentbracket"
      )
        set_is_inside(false);
      else set_is_inside(true);
    }

                                    ...
                                    <>
                                        {
                                            friends.slice(0, (friends.length - 2)).map((request, index) => (
                                                <FriendCard key={index} isLastTwoElements={false} secondUsername={request.friend_username} avatar={request.avatar}></FriendCard>
                                            ))
                                        }
                                        {
                                            friends.slice(-2).map((request, index) => (
                                                <FriendCard key={index} isLastTwoElements={friends.length > 2 ? true : false} secondUsername={request.friend_username} avatar={request.avatar}></FriendCard>
                                            ))
                                        }
                                    </>
                            }
                        ...
                        <div className="FriendRequests">
                            {
                                recievedRequests.length === 0 ?
                                    ...
                                    recievedRequests.map((request, index) => (
                                        <RecievedFriendReqCard key={index} secondUsername={request.username} send_at={request.send_at} avatar={request.avatar}></RecievedFriendReqCard>
                                    ))
                            }
                        </div>)}
                        ...
                        <div className="SentRequests">
                            {
                                sentRequests.length === 0 ?
                                    ...
                                    sentRequests.map((request, index) => (
                                        <SentFriendReqCard key={index} secondUsername={request.username} send_at={request.send_at} avatar={request.avatar}></SentFriendReqCard>
                                    ))
                            }
                        </div>)}

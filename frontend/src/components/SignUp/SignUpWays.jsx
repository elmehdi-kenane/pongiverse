...
function SignUpWays() {
	...
	const verify_email = async (email, picture) => {
		...
		const response = await fetch(`https://${import.meta.env.VITE_IPADDRESS}:8000/auth/checkemail/`, {
			method: "POST",
			headers: {
	...
	useEffect(() => {
		const google_get_data = async () => {
			const response = await fetch(`https://${import.meta.env.VITE_IPADDRESS}:8000/auth/sign-up-google-login-get-token/`, {
				method: "POST",
				headers: {
					'Content-Type': 'application/json',
				},
	...
	useEffect(() => {
		const intra_get_data = async () => {
			const response = await fetch(`https://${import.meta.env.VITE_IPADDRESS}:8000/auth/sign-up-intra-login-get-token/`, {
				method: "POST",
				headers: {
					'Content-Type': 'application/json',
				},
	...
	const handleGoogleClick = () => {
		const getGoogleUrl = async () => {
			const response = await fetch(`https://${import.meta.env.VITE_IPADDRESS}:8000/auth/sign-up-google-get-url`, {
				method: "GET",
				headers: {
					'Content-Type': 'application/json',
				},
	...
	const handleIntraClick = () => {
		const getIntraUrl = async () => {
			const response = await fetch(`https://${import.meta.env.VITE_IPADDRESS}:8000/auth/sign-up-intra-get-url/`, {
				method: "GET",
				headers: {
					'Content-Type': 'application/json',
				},

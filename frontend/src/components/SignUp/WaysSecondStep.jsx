...
function WaysSecondStep() {
	...
	const handleSubmit = (e) => {
		...
		if (Object.keys(validationErrors).length === 0)
		{
			...
			axios.get(nextdata.avatar, { responseType: 'blob' })
			.then(response => {
			...
			<div className={styles["mainPage"]}>
				...
				<div className={styles["bodyPage"]}>
					...
					<form className={styles["signUpForm"]} onSubmit={handleSubmit} noValidate>
						<input className={styles["inputs"]} type="text" value={nextdata.username} name='username' onChange={handleInputChange} placeholder="enter a username"/>
						{errors.username && <span>{errors.username}</span>}
						{exist && <span>Username already used</span>}
						<input className={styles["inputs"]} type="password" name='password' value={nextdata.password} onChange={handleInputChange} placeholder="enter a password"/>
						{errors.password && <span>{errors.password}</span>}
						<input className={styles["inputs"]} type="password" name='confirmPassword'  onChange={handleInputChange} placeholder="confirm your password"/>

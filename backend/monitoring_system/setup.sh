...
if [ -d "./tools/prometheus" ]; then
    ...
    wget -q https://github.com/prometheus/prometheus/releases/download/v2.55.1/prometheus-2.55.1.darwin-amd64.tar.gz
    echo -e "\033[1;34m ============ Prometheus downloaded 🔰 ============\033[0m"
    mv prometheus-2.55.1.darwin-amd64.tar.gz prometheus.tar.gz
    tar xfz prometheus.tar.gz

    mv prometheus.tar.gz archive
    mv prometheus-2.55.1.darwin-amd64 tools/prometheus
fi
...
if [ -d "./tools/alertmanager" ]; then
    ...
    wget -q https://github.com/prometheus/alertmanager/releases/download/v0.28.0-rc.0/alertmanager-0.28.0-rc.0.darwin-amd64.tar.gz
    echo -e "\033[1;34m ============ Alertmanager downloaded 🔰 ============\033[0m"
    mv alertmanager-0.28.0-rc.0.darwin-amd64.tar.gz alertmanager.tar.gz
    tar xfz alertmanager.tar.gz

    mv alertmanager.tar.gz archive
    mv alertmanager-0.28.0-rc.0.darwin-amd64 tools/alertmanager
fi
...
if [ -d "./tools/node_exporter" ]; then
    ...
    wget -q https://github.com/prometheus/node_exporter/releases/download/v1.8.2/node_exporter-1.8.2.darwin-amd64.tar.gz
    echo -e "\033[1;34m ============ NodeExporter downloaded 🔰 ============\033[0m"
    mv node_exporter-1.8.2.darwin-amd64.tar.gz node_exporter.tar.gz
    tar xfz node_exporter.tar.gz

    mv node_exporter.tar.gz archive
    mv node_exporter-1.8.2.darwin-amd64 tools/node_exporter
fi
...
if [ -d "./tools/grafana" ]; then
    ...
    wget -q https://dl.grafana.com/enterprise/release/grafana-enterprise-11.3.0+security-01.darwin-amd64.tar.gz
    echo -e "\033[1;34m ============ Grafana downloaded 🔰 ============\033[0m"
    mv grafana-enterprise-11.3.0+security-01.darwin-amd64.tar.gz grafana.tar.gz
    tar xfz grafana.tar.gz

    mv grafana.tar.gz archive
    mv grafana-v11.3.0+security-01 tools/grafana
